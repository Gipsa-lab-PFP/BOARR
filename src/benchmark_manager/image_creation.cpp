...
cv::Point Test::worldToImage(double world_x, double world_y)
{
...
void Test::draw_cross(cv::Mat &img, cv::Point position, cv::Scalar Color, int length, int width, int orientation )
{
    if ( orientation == 0 )
    {
...
void Test::addFrameSpecificContent()
{
...
void Test::drawTable(cv::Mat &img)
